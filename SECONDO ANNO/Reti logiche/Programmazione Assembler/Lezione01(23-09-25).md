...
> - preleva un'istruzione macchina (insieme di 0 e 1) dalla memoria;
> - La esegue
...
Il passaggio dal linguaggio **Assembly** al **linguaggio macchina** si chiama _assemblaggio_ ed è compito di un programma chiamato _**Assembler**_.
...
- Mancano i costrutti di controllo di flusso strutturato tipici dei linguaggi ad alto livello:
	...
	- for...to...do
	- switch...case
- Solo costrutti primitivi (salti condizionati e incondizionati) che permettono di realizzare i costrutti di controllo di flusso strutturato che vengono chiamati `Jump` .
...
Questo modello concettuale divide la macchina in quattro macro-sezioni fondamentali interconnesse.
#### Processore
Il processore è il "cervello" operativo della macchina. Il suo lavoro si basa su un ciclo continuo descritto in basso nello schema:
1. "**Preleva istruzione**" $\rightarrow$ legge un comando dalla memoria;
2. "**Esegui istruzione**" $\rightarrow$ svolge l'azione richiesta.
Al suo interno contiene componenti specializzati nei calcoli:
- **ALU (Arithmetic Logic Unit):** L'Unità Aritmetico Logica esegue le operazioni matematiche di base (addizioni, sottrazioni) e le operazioni logiche. L'appunto a penna specifica "IN Z", a indicare che l'ALU elabora tipicamente i numeri interi.
- **FPU (Floating Point Unit):** È il modulo dedicato ai calcoli in virgola mobile ("Floating Point"), ovvero le operazioni matematiche più complesse sui numeri reali, indicati dall'appunto in rosso con la lettera "R".
#### La Memoria Principale
La **MEMORIA PRINCIPALE** (nota comunemente come RAM) è lo spazio di lavoro ad alta velocità del sistema. Il suo ruolo è conservare temporaneamente i **"Dati e programmi"** in uso. Il processore deve accedere continuamente a questo blocco per sapere quali istruzioni eseguire e per recuperare i dati grezzi da elaborare.
#### Sottosistema di Ingresso/Uscita (I/O)
Questo blocco permette al calcolatore di interagire con il mondo esterno, permettendo l'inserimento e la visualizzazione dei risultati.
- **DISPOSITIVI:** Sono le periferiche fisiche come mouse, tastiera o monitor, che gestiscono informazioni umane come **"Immagini, suoni, etc."**.
- **INTERFACCE:** Sono i circuiti che fanno da "traduttori" tra i dispositivi e il computer. Trasformano le azioni fisiche o i segnali analogici in sequenze di **"bit"** (l'unità base dell'informazione digitale, formata da 0 e 1) comprensibili per il sistema centrale.
#### Il BUS (Rete di Interconnessione)
Il **BUS** è la **RETE di INTERCONNESSIONE** del sistema. È il canale di comunicazione principale, l'"autostrada" elettronica su cui viaggiano i dati. Collega tutti e tre gli altri blocchi (Processore, Memoria e I/O), permettendo ai bit di viaggiare rapidamente dall'uno all'altro per far funzionare la macchina in armonia.
...
- Su N bit si rappresentano 2N numeri interi, ($-2^{N-1}$; $2^{N-1}-1$).
...
- Il numero intero x viene rappresentato con la stringa di bit X che corrisponde al numero naturale:
  ...
  - Se x < 0, X è la rappresentazione in base 2 di $2^N$ + x.
$$
\begin{cases}
x & x \geq 0 \\
2^N + x & x < 0
\end{cases}
$$
- Purchè x sia un intero rappresentabile, i cui valori sono compresi nell'intervallo ($2^{N-1}$; $2^{N-1}-1$).

>[!Note]
>Quando si parla di numeri in C2, tenere sempre a mente questo disegno:
>
> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850 550" style="background-color: white; font-family: Arial, sans-serif;">
>   <!-- Definizioni per le frecce e i punti -->
>   <defs>
...
> 
>   <!-- Assi Cartesiani -->
>   <line x1="120" y1="450" x2="750" y2="450" stroke="black" stroke-width="1.5" marker-end="url(#arrow)" />
>   <line x1="450" y1="450" x2="450" y2="30" stroke="black" stroke-width="1.5" marker-end="url(#arrow)" />
> 
>   <!-- Etichette degli Assi -->
>   <text x="740" y="475" font-size="16" font-style="italic">x</text>
>   <text x="425" y="40" font-size="16" font-style="italic">Y</text>
>   <text x="450" y="490" font-size="18" text-anchor="middle">Numero intero</text>
>   <text x="410" y="250" font-size="18" transform="rotate(-90 410 250)" text-anchor="middle">rappresentazione</text>
> 
>   <!-- Linee Tratteggiate (Proiezioni) -->
>   <line x1="250" y1="450" x2="250" y2="250" stroke="gray" stroke-dasharray="5,5" stroke-width="1.5" />
>   <line x1="250" y1="250" x2="450" y2="250" stroke="gray" stroke-dasharray="5,5" stroke-width="1.5" />
>   <line x1="650" y1="450" x2="650" y2="250" stroke="gray" stroke-dasharray="5,5" stroke-width="1.5" />
>   <line x1="650" y1="250" x2="450" y2="250" stroke="gray" stroke-dasharray="5,5" stroke-width="1.5" />
> 
>   <!-- Curve del Grafico (Perfettamente Parallele) -->
>   <!-- Linea Verde (Numeri Positivi): Pendenza = -1 -->
>   <line x1="450" y1="450" x2="650" y2="250" stroke="#2ca02c" stroke-width="3" marker-start="url(#dot)" marker-end="url(#dot)" />
>   
>   <!-- Linea Rossa (Numeri Negativi): Pendenza = -1 -->
>   <line x1="250" y1="250" x2="450" y2="50" stroke="#d62728" stroke-width="3" marker-start="url(#dot)" marker-end="url(#dot)" />
> 
>   <!-- Valori sull'Asse X -->
>   <text x="250" y="475" font-size="16" text-anchor="middle">-2<tspan dy="-10" font-size="12">N-1</tspan></text>
>   <text x="650" y="475" font-size="16" text-anchor="middle">2<tspan dy="-10" font-size="12">N-1</tspan><tspan dy="10">-1</tspan></text>
> 
>   <!-- Valori sull'Asse Y e Pattern Binari -->
>   <text x="460" y="55" font-size="16">2<tspan dy="-10" font-size="12">N</tspan><tspan dy="10">-1</tspan> <tspan font-size="14">(111...111)</tspan></text>
>   <text x="460" y="240" font-size="16">2<tspan dy="-10" font-size="12">N-1</tspan> <tspan dy="10" font-size="14">(100...000)</tspan></text>
>   <text x="460" y="275" font-size="16">2<tspan dy="-10" font-size="12">N-1</tspan><tspan dy="10">-1</tspan> <tspan font-size="14">(011...111)</tspan></text>
>   <text x="460" y="445" font-size="14">(000...000)</text>
> 
>   <!-- Etichetta punto in alto a sinistra per i negativi -->
>   <text x="240" y="255" font-size="14" text-anchor="end">(-2<tspan dy="-10" font-size="10">N-1</tspan><tspan dy="10">, pattern (100...000))</tspan></text>
> 
>   <!-- Riquadri Laterali per MSB -->
>   <!-- Riquadro Superiore: MSB = 1 -->
>   <rect x="680" y="50" width="35" height="195" fill="none" stroke="black" stroke-width="1.2" />
>   <text x="703" y="147" font-size="14" transform="rotate(90 703 147)" text-anchor="middle">MSB = 1</text>
>   
>   <!-- Riquadro Inferiore: MSB = 0 -->
>   <rect x="680" y="255" width="35" height="195" fill="none" stroke="black" stroke-width="1.2" />
>   <text x="703" y="352" font-size="14" transform="rotate(90 703 352)" text-anchor="middle">MSB = 0</text>
> </svg>
> 
> **Perché questo grafico è fondamentale:**
> Mostra visivamente come i pattern binari a N bit vengono proiettati sui numeri interi. 
> - **Linea verde (MSB = 0):** Rappresenta i numeri positivi. Si parte dal pattern di bit tutti a zero e si sale linearmente fino al valore massimo `2^(N-1) - 1`.
> - **Linea rossa (MSB = 1):** Rappresenta i numeri negativi. Avviene un salto ("wrap-around"): i pattern binari successivi vengono traslati nell'asse negativo, partendo dal valore più basso `-2^(N-1)` fino ad arrivare a `-1` (corrispondente al pattern con tutti 1).
> Questo chiarisce l'asimmetria del C2: l'intervallo negativo contiene sempre un numero in più rispetto a quello positivo.

- Versione equivalente: ovvero il resto della divisione di |X| per $2^N$.
$$ X = |X|_{2^N} $$
...
<div style="display: grid; grid-template-columns: 2fr 1fr; padding: 10px;">
    ...
    <div style="border: 4px solid black; width: fit-content; margin-top: 20px; margin-bottom: 20px; margin: 0;">
      <img src="calcolatore_struttura.png" alt="Schema memoria" style="display:flex; width:250px; height:350px;">
    </div>
</div>
...
- Processore: accede in lettura o scrittura a una porta di I/O specifica usando due particolari istruzioni (_in_ o _out_):
...
> <br>
> <div style="display:flex; flex-direction:column; align-items:center;"> <img src="../images/processore.png" alt="processore" style="max-width:60%;"><p style="text-align:center;">Questa immagine rappresenta i registri di un processore, elementi fondamentali dell'architettura di un computer utilizzati per l'archiviazione temporanea di dati e indirizzi durante l'esecuzione di istruzioni.</p></div>
> 
>  - La parola 'E' prima di ogni registro significa **_extended_**
<div style="page-break-before: always;"></div>
...
1. EIP vale 0xFFFF0000
   2. La prima istruzione si trova li'
   3. Le celle da 0xFFFF0000 in poi devono essere implementate in ROM
4. In EF i flag che ci interessano valgono 0, ovvero:
   5. OF = 0
   6. SF = 0
   7. ZF = 0
   8. CF = 0
