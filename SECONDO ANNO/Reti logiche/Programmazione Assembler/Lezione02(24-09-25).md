...
$$ \text{OPCODEsuffix source, dest}$$
$$\text{ADD \%BX, pluto} \leftrightarrow \text{pluto += \%BX}$$
...
- Se due, il primo e' **sorgente**, il secondo e' **destinatario**.
  - Il sorgente non viene modificato dall'istruzione
  - il destinatario si e viene sostituito con il risultato dell'elaborazione.
- Se uno, può essere **sorgente** o **destinatario** a seconda dell'istruzione.
- Se ci sono due operandi **sono della stessa lunghezza** (tranne qualche eccezione).
---
...
![Esempio di programma (cont)|700](primoprogramma.png)
...
```asm6502
  ...
  CMPL $0x00000000, %EAX // Compara il valore che è presente all'indirizzo 0x00000000 (0), com il valore di EAX.
  
  JE %EIP+0x07  // Jump if equal da qualche parte. se dentro if go 1 salta a EIP+7 ovvero 206 + 7 = 213
  
  SHRL %EAX  // Sposta di un bit, l'LSB va in CF
...
L’indirizzamento di memoria è il modo in cui un processore accede ai dati in memoria. Ogni cella di memoria ha un indirizzo univoco, e le istruzioni del processore devono specificare dove leggere o scrivere i dati. In altre parole, l’indirizzamento dice al processore “da quale indirizzo prendere il dato” o “dove memorizzarlo”. E' necessasrio specificare un indirizzo di memoria a 32 bit
...
>[!tip] Sintassi
> $\text{OPCOODEsfx} \ \pm\text{disp(base,indice,scale)}$
...
3. **Indirizzamento con displacement e registro di modifica**: In questa modalità, l'indirizzo effettivo in memoria viene calcolato sommando un valore costante, chiamato _displacement_ (o offset/spiazzamento), al contenuto di un registro della CPU (che funge da registro base o registro indice).
	- **Formula:** `EA = (Registro) + displacement`
	- **A cosa serve:** È utilissimo per accedere ai campi di una struttura dati complessa (come le `struct` in C). Il registro punta all'inizio della struttura, mentre il displacement indica la distanza (fissa) del campo specifico dall'inizio della struttura. Può essere usato anche per scorrere un array se il displacement rappresenta la base dell'array e il registro fa da contatore.
4. **Indirizzamento bimodificato senza displacement**: Questa modalità calcola l'indirizzo effettivo sommando semplicemente il contenuto di _due_ registri diversi, di solito un "registro base" e un "registro indice", senza l'aggiunta di alcuna costante (ovvero con un displacement pari a zero).
	- **Formula:** `EA = (Registro Base) + (Registro Indice)`
	- **A cosa serve:** È l'ideale quando sia la posizione di partenza dei dati, sia l'indice a cui si vuole accedere sono calcolati dinamicamente durante l'esecuzione del programma. Un esempio tipico è lo scorrimento di array bidimensionali (matrici), dove un registro tiene traccia della riga e l'altro della colonna.
5. **Indirizzamento bimodificato con displacement**: È la combinazione delle due tecniche precedenti e risulta essere la modalità di indirizzamento più potente (ma anche la più complessa da calcolare per l'hardware). L'indirizzo effettivo si ottiene sommando il contenuto di due registri (base e indice) a un valore costante di displacement.
	- **Formula:** `EA = (Registro Base) + (Registro Indice) + displacement`
	- **A cosa serve:** Viene utilizzato in strutture dati molto complesse. Per esempio, immagina di avere un array di strutture (`struct`). Il registro base potrebbe puntare all'inizio dell'array, il registro indice scorrere per selezionare una specifica struttura nell'array (moltiplicato per la dimensione della struttura), e il displacement servirebbe per accedere a un singolo campo specifico _all'interno_ di quella singola struttura trovata.
---
...
- **Istruzioni operative**:
  - **Istruzioni di trasferimento dati**: Spostano le informazioni tra i registri della CPU e la memoria principale, o da un registro all'altro, senza alterarne il valore. Esempi classici sono il caricamento dalla memoria a un registro (`LOAD`), il salvataggio da un registro alla memoria (`STORE`) o la copia tra registri (`MOV`).
  - **Istruzioni aritmetiche**: Eseguono le operazioni matematiche fondamentali (addizione, sottrazione, moltiplicazione, divisione) sui dati. Vengono elaborate dall'ALU (Arithmetic Logic Unit) e spesso aggiornano i bit di stato (flag) del processore per indicare se il risultato è zero, negativo o se ha generato un riporto.
  - **Istruzioni logiche**: Applicano gli operatori dell'algebra di Boole (AND, OR, NOT, XOR) operando bit a bit sui registri. Sono indispensabili per testare, impostare o azzerare singoli bit all'interno di una parola di dati (operazioni di "mascheramento").
  - **Istruzioni di traslazione/rotazione_**: Spostano lateralmente l'intera sequenza di bit all'interno di un registro. Nelle _traslazioni_ (shift), i bit che escono vengono scartati e vengono inseriti degli zeri (o replicato il bit di segno), un metodo molto efficiente per moltiplicare o dividere rapidamente per potenze di due. Nelle _rotazioni_, il bit che esce da un'estremità rientra dall'estremità opposta, senza perdita di informazioni.
- **Istruzioni di controllo**: Modificano il normale flusso sequenziale di esecuzione delle istruzioni intervenendo sul Program Counter (PC). Includono le istruzioni di salto e di chiamata a sottoprogramma.
  - **Istruzioni di salto**: Deviano l'esecuzione verso uno specifico indirizzo di memoria. I salti _incondizionati_ avvengono sempre, mentre i salti _condizionati_ vengono eseguiti solo se si verifica un determinato requisito logico (es. salta solo se il risultato dell'ultima operazione è zero o se c'è stato un riporto).
  - **Istruzioni per la gestione di sottoprogrammi**: Permettono l'esecuzione di blocchi di codice isolati (funzioni o procedure). L'istruzione di chiamata (`CALL`) salva l'indirizzo dell'istruzione successiva (solitamente in una struttura chiamata Stack) e salta alla procedura; l'istruzione di ritorno (`RET`) recupera quell'indirizzo per riprendere l'esecuzione esattamente da dove era stata interrotta.
---
